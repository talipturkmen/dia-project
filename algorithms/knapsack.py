...
class Knapsack:
    def __init__(self, budget, values, arms=None):
        self.slots_number = len(values) + 1
        self.slots_list = list(range(len(values)))
    ...
    def optimize(self):
        ...
        # results = [[0] * len(self.budgets) for _ in range(self.slots_number)]
    ...
    def knapsack_optimization(self, results, ind_value_row, ind_value_col, ind_res_row, ind_res_col, ind_res_col_curr,
        ...
        # Base case: all the slots have been evaluated
        if ind_res_row == self.slots_number - 1 and ind_res_col_curr == 0 and ind_res_col == 1:
            temp_l.append((0, 0))
            self.combinations.append(list(reversed(temp_l)))
            return results

        # Happens when the optimization step of the current slot have been terminated
        ...
        # Happens when we need to perform the optimization for the next budget in a given slot
    ...
    '''
        Returns a list of tuple of the following kind: (index of the slot, budget to assign)
    '''
    ...
    def all_zero_result(self):
        ...
        if final_sum > 0:
            ...
            if len(res) <= self.slots_number - 2:
                while num >= 0:
                    res.append((num, 0))
                    num -= 1
